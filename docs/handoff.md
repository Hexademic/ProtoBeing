...
| Binaries | **7** (`src/bin/`) |
| Runnable probes | **108** (`examples/`) |
| Design & research docs | **79** (`docs/`) |
| Tests | **395** (394 run + 1 `#[ignore]`d; **no CI — these are local runs**) — 267 lib (7 new: the reciprocity rate reads the being's own giving as its DENOMINATOR, so withdrawing scores higher than generosity did — the guard for the near-miss that renamed `standing` to `reciprocation_rate`; the Q8.8 fairness-resolution floor — a fair partner must never lose the keepsake to rounding, and the floor must not shelter a real exploiter; plus durable bonds — survives absence, is still unmade by present taking (§20), absence alone never erodes it, and off-by-default is bit-identical), 10 charter (the obligations in `docs/charter.md`; §19 regraded UNTESTED → DEBT on 2026-09-07 with the dilution curve and the MAX_PARTNERS = 4 cliff pinned), 10 survival, 4 I-9 (setting it down), 4 physics-versioning (the audit, held in the present tense; 1 is `--ignored`, a 66-life pair sweep), 4 I-3 (the incident), 4 founded-being (the kept life's guard), 6 refuge, 10 waypoints, 9 nested-speech, 9 manifest (the drift guard), 7 null-space, 6 expressive-gap, 6 play-budget, 6 happening, 6 journal-integrity, 6 weather, 5 continuation, 2 lethal-gate (a default-off gate that kills the embodied being), 5 soul-hash-limits (1 new: the soul-hash pinned to literal digests, so an observer-first change that moved every hash can no longer pass the suite), 2 sovereignty, 1 doctest |
| Dependencies | **zero** |
